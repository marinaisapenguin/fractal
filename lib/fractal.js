(function(){
  ...
  //Qs: all functions defined on prototype? make private functions except for update?
  ...
  Fractal.prototype.setMaxEscapeTime = function(maxEscapeTime){
    if (maxEscapeTime > 1793) {
      this.maxEscapeTime = 1793;
    } else if (maxEscapeTime < 15) {
      this.maxEscapeTime = 15;
    } else {
      this.maxEscapeTime = Math.floor((maxEscapeTime - 1) / 7) * 7 + 1;
    }
  };
  ...
  Fractal.prototype.rgbNum = function(escapeTime){
    if (escapeTime <= 2) {
      return [0, 0, 0];
    } else if (escapeTime >= this.maxEscapeTime - 1) {
      return [0, 25, 0];
    }
