(function(){
  ...
  Fractal.prototype.draw = function(imageData){
    if (imageData) {
      ...
      pixels = function addCommas(intNum) {
      ...
      this.ctx.fillText("calculating " + pixels + " pixels...",75 * devicePixelRatio, 165 * devicePixelRatio);

      setTimeout(function(){
        imageData = this.drawToImageData();
        this.ctx.putImageData(imageData, 0, 0);
        // this.ctx.font="300 1.25em Helvetica";
        // this.ctx.fillStyle = "#c0c0c0";
        this.ctx.fillText("click to zoom in",75 * devicePixelRatio,75 * devicePixelRatio);
        this.ctx.fillText("option-click to zoom out",75 * devicePixelRatio,105 * devicePixelRatio);
        this.ctx.fillText("shift-click to reset",75 * devicePixelRatio,135 * devicePixelRatio);


      }.bind(this), 30);
    }

    // this.ctx.font="5em Verdana";
    // this.ctx.fillStyle = "#c0c0c0";
    // this.ctx.fillText("hiii ",20,20);
